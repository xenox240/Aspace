...
module.exports = class UnknownCommandCommand extends Command {
	...
	run(msg) {
		...
		return msg.reply(stripIndents`
			Commande inconnue. Utilise ${this.client.registry.commands.get('help').usage()} pour voir la liste de commande.

			${results.length ? `vouliez-vous dire: ${results.slice(0, 5).map(c => `\`${c}\``).join(', ')}` : ''}
		`);
	}
